...
skiptallydrawer:
	...
	if (timer)
		V_DrawCenteredString(BASEVIDWIDTH/2, 188, V_YELLOWMAP, va("start in %d seconds", timer/TICRATE));
...
void Y_Ticker(void)
{
	...
	// Zenya's Server Mod
	// Add check to see if we need to warp to another map based on current map and (not yet) next map.
	if (timer)
	{
		if (timer < 5)
		{
			if (WarpCheck(gamemap, nextmap))
			{
				timer = 0;
				return;
			}
