...
mobj_t *P_SpawnMobj(fixed_t x, fixed_t y, fixed_t z, mobjtype_t type)
{
	...
	switch (mobj->type)
	{
		...
		case MT_EGGROBO1:
			...
			break;
		// Zenya's Server Mod
		// Potential Aerial Garden Bee Fix 1: Set Beehive max bees to 1, lowers the total amount of possible bees in level by 80%. (Does work kind of, don't know if it fixes the issue.)
		// Potential Aerial Garden Bee Fix 2: Set Beehive health to 0 to immediately destroy them. (Doesn't work. Causes ghost bees.)
		// Potential Aeria Garden Bee Fix 3: Set Beehive objecttype to Ring. Should replace them with rings. (Doesn't work. Might not be the right place to do it.)
		case MT_HIVEELEMENTAL:
			mobj->extravalue1 = 5; // Default
			// mobj->extravalue1 = 1; // Fix 1
			// mobj->health = 0; // Fix 2
			// mobj->type = MT_RING; // Fix 3
			break;
		case MT_SMASHINGSPIKEBALL:
